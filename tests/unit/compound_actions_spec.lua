...
local function footprint(parts)
...
describe("schema-v2 compound actions", function()
  it("constructs action drafts with v2 geometry and tagged tuples", function()
    ...
    value = apply(value, {
      type = "add_room",
      room = {
        id = "room-l", name = "L room", origin_mm = { 0, 0 },
        footprint = footprint({
    ...
    value = apply(value, {
      type = "add_furniture",
      furniture = {
        id = "furniture-l", room_id = "room-l", template_id = "custom:sectional",
        name = "Sectional", category = "seating", position_mm = { 300, 300 },
        anchor2_mm = { 100, 100 }, height_mm = 850, rotation_deg = 0,
        footprint = footprint({
      ...
      custom_template = {
        ...
        default_anchor2_mm = { 100, 100 }, default_height_mm = 850,
  ...
  it("moves and duplicates furniture through position_mm without flattening it", function()
    local value = plan_v2()
    add_room(value, {
      id = "room-main", name = "Main", origin_mm = { 0, 0 },
      footprint = footprint({ part("part-main", 0, 0, 2000, 2000) }),
    })
    local source = add_furniture(value, {
      id = "furniture-source", room_id = "room-main", template_id = "builtin:custom-rectangle",
      name = "L couch", category = "seating", position_mm = { 500, 500 },
      anchor2_mm = { 100, 100 }, height_mm = 800, rotation_deg = 0,
      footprint = footprint({
        part("part-main", 0, 0, 200, 100),
        part("part-return", 0, 100, 100, 100),
      }),
    ...
    value = apply(value, {
      type = "move_furniture", id = source.id, position_mm = { 600, 650 }, exact = true,
    })
    ...
    local rejected, err = actions.apply(value, {
      type = "move_furniture", id = source.id, center_mm = { 1, 2 }, exact = true,
    })
    assert_equal(nil, rejected)
    assert_equal("INVALID_ACTION", err.code)

    value = apply(value, {
      type = "duplicate_furniture", id = source.id, new_id = "furniture-copy", step_mm = 50,
    })
  ...
  it("auto-places a duplicated compound room from the complete footprint", function()
    ...
    local source = add_room(value, {
      id = "room-source", name = "L room", origin_mm = { 0, 0 },
      footprint = footprint({
        part("part-main", 0, 0, 1000, 800),
        part("part-east", 1000, 0, 300, 300),
      }),
    ...
    value = apply(value, {
      type = "duplicate_room", id = source.id, new_id = "room-copy", cursor_mm = { 1500, 0 },
    })
  ...
  it("snaps compound doors only against doors on the same room part", function()
    ...
    value.doors[1] = model.new_door({
      id = "door-main", room_id = "room-main", part_id = "part-main", side = "south",
      offset_mm = 80, width_mm = 100,
    }, V2)
    value.doors[2] = model.new_door({
      id = "door-east", room_id = "room-main", part_id = "part-east", side = "south",
      offset_mm = 100, width_mm = 100,
    }, V2)

    value = apply(value, {
      type = "edit_door", id = "door-main", patch = { offset_mm = 90 },
  ...
  it("resizes canonical rectangles and rejects compound or custom-anchor geometry", function()
    local value = plan_v2()
    add_room(value, {
      id = "room-main", name = "Main", origin_mm = { 0, 0 }, size_mm = { 2000, 2000 },
    })
    add_furniture(value, {
      id = "furniture-main", room_id = "room-main", template_id = "builtin:custom-rectangle",
      name = "Desk", category = "work", position_mm = { 500, 500 },
      size_mm = { 200, 100, 700 }, rotation_deg = 0,
    })
    ...
    add_room(compound, {
      id = "room-l", name = "L", origin_mm = { 0, 0 },
      footprint = footprint({
        part("part-main", 0, 0, 1000, 800),
        part("part-east", 1000, 0, 300, 300),
      }),
    ...
    local changed, err = actions.apply(compound, {
      type = "resize_room", id = "room-l", size_mm = { 100, 100 },
    })
    ...
    add_room(custom_anchor, {
      id = "room-main", name = "Main", origin_mm = { 0, 0 }, size_mm = { 1000, 1000 },
    })
    add_furniture(custom_anchor, {
      id = "furniture-corner", room_id = "room-main", template_id = "builtin:custom-rectangle",
      name = "Corner anchored", category = "custom", position_mm = { 100, 100 },
      anchor2_mm = { 0, 0 }, height_mm = 100, rotation_deg = 0,
      footprint = footprint({ part("part-main", 0, 0, 100, 100) }),
    })
    changed, err = actions.apply(custom_anchor, {
      type = "resize_furniture", id = "furniture-corner", size_mm = { 200, 200, 100 },
    })
    assert_equal(nil, changed)
    assert_equal("COMPOUND_RESIZE_UNSUPPORTED", err.code)
  ...
  it("keeps attached doors stable and blocks unsafe L-room edits atomically", function()
    ...
    value.doors[1] = model.new_door({
      id = "door-l", room_id = "room-l", part_id = "part-horizontal", side = "north",
      offset_mm = 3000, width_mm = 900,
    }, V2)
    ...
    local enlarged = apply(value, {
      type = "edit_room", id = "room-l", patch = {
        footprint = assert(room_footprints.build({
          shape = "l_shape", width_mm = 4500, depth_mm = 3000,
          leg_width_mm = 1500, leg_depth_mm = 1200, missing_corner = "northeast",
        })),
      },
    })
    ...
    local blocked, err = actions.apply(value, {
      type = "edit_room", id = "room-l", patch = {
        footprint = assert(room_footprints.build({
          shape = "l_shape", width_mm = 3500, depth_mm = 3000,
          leg_width_mm = 1500, leg_depth_mm = 1200, missing_corner = "northeast",
        })),
      },
    })
    ...
    blocked, err = actions.apply(value, {
      type = "edit_room", id = "room-l", patch = {
        footprint = assert(room_footprints.build({
          shape = "l_shape", width_mm = 4000, depth_mm = 3000,
          leg_width_mm = 1500, leg_depth_mm = 1200, missing_corner = "northwest",
        })),
      },
    })
    assert_equal(nil, blocked)
