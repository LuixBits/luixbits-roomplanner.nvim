...
local function normalize_root(context, document)
  ...
  if not site then document.site = nil; return end
  site.north_deg = common.number(
    context, common.required(context, site, "north_deg", "$.site"), "$.site.north_deg", 0, 360
  )
  ...
  site.latitude_deg = common.number(
    context, common.required(context, site, "latitude_deg", "$.site"), "$.site.latitude_deg", -90, 90
  )
  site.longitude_deg = common.number(
    context, common.required(context, site, "longitude_deg", "$.site"), "$.site.longitude_deg", -180, 180
  )
  site.utc_offset_minutes = common.integer(
    context, common.required(context, site, "utc_offset_minutes", "$.site"),
    "$.site.utc_offset_minutes", -14 * 60, 14 * 60, 14 * 60
  )
  document.site = site
end
