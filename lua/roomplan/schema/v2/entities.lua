...
function M.normalize_room(context, source, path, room_footprints)
  ...
  local runtime
  result.footprint, runtime = footprint.normalize(
    context,
    common.required(context, result, "footprint", path),
    path .. ".footprint"
  )
...
function M.normalize_furniture(context, source, path)
  ...
  result.anchor2_mm = footprint.normalize_anchor(
    context,
    common.required(context, result, "anchor2_mm", path),
    path .. ".anchor2_mm"
  )
  local runtime
  result.footprint, runtime = footprint.normalize(
  ...
  result.height_mm = common.dimension(
    context,
    common.required(context, result, "height_mm", path),
    path .. ".height_mm"
  )
...
function M.normalize_template(context, source, path)
  ...
  result.default_height_mm = common.dimension(
    context,
    common.required(context, result, "default_height_mm", path),
    path .. ".default_height_mm"
  )
  return result
end
