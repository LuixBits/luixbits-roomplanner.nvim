...
function M.commit(context, patch, expected_revision, opts)
  ...
  if context.bufnr and vim.api.nvim_buf_is_loaded(context.bufnr) then
    ...
    if expected_revision and current ~= expected_revision.text and current ~= patch.text then
      return nil, util.err("SOURCE_CONFLICT", "standalone source buffer changed after RoomPlan opened", {
    ...
    if revision.disk and revision.disk.exists then
      ...
      if not durable then
        return nil, util.err("SOURCE_POST_WRITE_INVALID", "durable standalone file is invalid after write", { cause = durable_err })
      end
      if not codec.deep_equal(durable, parsed) then
        return nil, util.err("SOURCE_POST_WRITE_DIVERGED", "source buffer and durable standalone file contain different models")
      end
    end
