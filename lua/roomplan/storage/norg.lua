...
local function decode_document(text)
  ...
  if document == nil then error((err and err.message) or "malformed JSON") end
  return document
end

local function discover(text) return scan.discover(text, decode_document) end
...
function M.load(context)
  ...
  if not found then return nil, discover_err end
...
function M.serialize(model) return schema.encode(model, { final_newline = false }) end
...
local function indent_payload(payload, indent)
  if not indent or indent == "" then return payload end
...
function M.commit(context, patch, expected_revision, opts)
  ...
  if not found then return nil, discover_err end
  if found.kind ~= "found" then return nil, util.err("NORG_PLAN_MISSING", "RoomPlan block disappeared before save") end
  if
    expected_revision
  ...
  if replaced ~= current then source.set_buffer_text(context.bufnr, replaced) end
  if opts.write ~= false then
    local written, write_err = source.write_buffer(context.bufnr)
    if not written then return nil, write_err, { staged = true, payload = patch.payload } end
  end
...
function M.initialize(context, model, opts)
  ...
  if not result then return nil, discover_err end
  ...
  if not payload then return nil, encode_err end
