  ...
  flow:input(
    ...
    function(value)
      ...
      if ok and units.parse then
        ...
        if not parsed then err = "expected a number" end
      end
      if not parsed then
        compat.notify(type(err) == "table" and err.message or tostring(err), vim.log.levels.ERROR)
        flow:retry(function(current) M.measurement(current, opts, callback) end)
        return
      end
  ...
  flow:input(
    ...
    function(value)
      ...
      if not number or number % 1 ~= 0 or (opts.min and number < opts.min) or (opts.max and number > opts.max) then
        ...
        flow:retry(function(current) M.integer(current, opts, callback) end)
        return
      end
...
function M.confirm(session, kind, prompt, choices, callback)
  ...
  if not flow then return nil, err end
