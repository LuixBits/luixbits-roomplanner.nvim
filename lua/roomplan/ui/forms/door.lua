...
local function edge_length(draft, context)
  ...
  return (draft.side == "north" or draft.side == "south") and room.size_mm[1] or room.size_mm[2]
end
...
local function connections(context, draft)
  ...
  for _, other in ipairs(common.model(context).rooms or {}) do
    if other.id ~= room.id then
      local shared = adjacency.between(room, other)
...
function M.add(session, opts)
  ...
  local spec = {
    ...
    on_change = function(key, value, _, draft)
      if key == "connects_to_room_id" then
